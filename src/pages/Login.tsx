...
export function Login() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // on successful login, AuthContext will update and unblock the ProtectedRoute/Navigate to root
    } catch (err: any) {
        ...
        <div className="flex flex-col items-center mb-8 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-white/50 text-sm">
              Log in to GentlGYM to continue tracking your progress
            </p>
          </div>
        </div>
        ...
        <div className="bg-white/[0.03] border border-white/[0.08] backdrop-blur-xl rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleEmailLogin} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-white text-black hover:bg-white/90 disabled:opacity-50 disabled:cursor-not-allowed border border-transparent rounded-2xl py-3.5 px-4 font-semibold text-sm transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_25px_rgba(255,255,255,0.2)] active:scale-[0.98] flex items-center justify-center gap-2"
            >
              {loading ? (
